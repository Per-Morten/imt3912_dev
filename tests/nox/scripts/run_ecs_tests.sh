...
do
    ...
    do
        ...
        ./run_test.sh --test-name="${compiler}_${nr}" --results-folder="$resultsRoot/$PREFIX/memory_usage" --command="./build/bin/ecs_memory_usage -actor_amount $i --logrm warning" --save-program-output=yes;
        ((i=i*2));
        ((count++));
    done
